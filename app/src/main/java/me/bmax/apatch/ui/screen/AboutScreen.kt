...
@Composable
private fun rememberFluidAnimationTime(isActive: Boolean): Float {
...
private fun androidx.compose.ui.graphics.drawscope.DrawScope.drawFluidGradientField(
    animationTime: Float,
    colors: List<Color>,
    fieldSize: Size,
    ...
    drawRect(
        brush = Brush.linearGradient(
            colors = strengthenedColors.map { color ->
                color.copy(
                    alpha = if (translucentPalette) color.alpha * 0.72f else 0.58f
                )
            },
            start = Offset.Zero,
...
fun FluidGradientBackground(
    ...
    heroHeightDp: Dp = 340.dp,
    scrollFadeDistanceDp: Dp = 389.dp,
    parallaxFactor: Float = 0.12f,
) {
    val density = LocalDensity.current
    val fadeDistancePx = with(density) { scrollFadeDistanceDp.toPx() }
    val animationTime = rememberFluidAnimationTime(isActive = isActive)
    val darkTheme = androidx.compose.foundation.isSystemInDarkTheme()
    val gradientColors = animatedGradientColors(animationTime, darkTheme)

    val backgroundAlpha by remember(scrollOffsetY, fadeDistancePx) {
        derivedStateOf {
            (1f - (scrollOffsetY / fadeDistancePx)).coerceIn(0f, 1f)
        }
    }
    ...
    Canvas(
        modifier = modifier
            .graphicsLayer {
                ...
                translationY = -scrollOffsetY * parallaxFactor
            }
    ) {
        drawFluidGradientField(
